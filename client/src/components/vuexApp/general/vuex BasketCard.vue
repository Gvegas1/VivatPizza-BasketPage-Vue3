...
export default {
  ...
  methods: {
    ...mapMutations({
      setPricesArray: "products/setPricesArray",
      deleteCard: "products/deleteCard",
    }),
    setDataForPricesArray() {
      if (!this.isSectionGifts) {
        this.setPricesArray({
          id: this.basketCardData.id,
          lastPrevPrice: this.lastPrevPrice
            ? this.lastPrevPrice
            : this.basketCardData.priceData.prevPrice,
          lastCurrentPrice: this.lastCurrentPrice
            ? this.lastCurrentPrice
            : this.basketCardData.priceData.currentPrice,
        });
      }
    },
    deleteCard() {
